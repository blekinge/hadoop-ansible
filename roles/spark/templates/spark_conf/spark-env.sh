...
export SPARK_DAEMON_JAVA_OPTS="-Dspark.root.logger=INFO,DRFA -Dspark.log.file=$SPARK_LOG_DIR/spark-$SPARK_IDENT_STRING-$(basename ${BASH_SOURCE[-1]} .sh)-$HOSTNAME.log -Dspark.syslog.file=spark-$SPARK_IDENT_STRING-$(basename ${BASH_SOURCE[-1]} .sh)-$HOSTNAME.log"
